...
function Logo() {
  return (
    <a href="#home" aria-label="TENDA BOX 8Q - الرئيسية" className="group flex items-center gap-3">
...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav aria-label="التنقل الرئيسي" className="mx-auto flex max-w-7xl items-center justify-between px-5 sm:px-8">
        <Logo />
      ...
      <AnimatePresence>
        {open && (
          <>
            {/* Opaque backdrop dims and blocks the page content underneath */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            {/* Full-screen opaque drawer sits above the backdrop, below the header bar */}
            <motion.div
              ...
              className="fixed inset-x-0 top-[72px] bottom-0 z-40 overflow-y-auto bg-ink lg:hidden"
            >
              <div className="mx-auto flex min-h-full max-w-md flex-col gap-6 px-6 pb-10 pt-6">
                ...
                <ul className="flex flex-col divide-y divide-white/5 rounded-3xl border border-white/10 bg-white/[0.02]">
                ...
                <div className="rounded-3xl border border-gold/20 bg-gold/[0.06] p-5">
                  ...
                  <p className="mt-1 text-base font-bold text-white">طنجة · على مدار الساعة</p>
                </div>
